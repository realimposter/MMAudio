...
def main():
    ...
    # If user sets device=cuda, verify CUDA is actually available
    if args.device.startswith('cuda') and not torch.cuda.is_available():
        log.warning("CUDA is not available on this system; falling back to CPU.")
        device = torch.device('cpu')

    # Choose float precision (FP32 or bfloat16)
